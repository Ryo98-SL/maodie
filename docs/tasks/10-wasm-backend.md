...
Host logging handoff: `core.log("...")` is recognized by symbol path `core.log` and lowered to debug chunk imports. `debug_string(ptr, len)` writes a string chunk, `debug_i32(value)` and `debug_bool(value)` write scalar chunks, and `debug_log_end()` flushes one complete log line. Formatted calls such as `core.log("value is {}", value)` require a literal format string and support direct `i32`、`bool`、`String` interpolation.
...
- `String` 是编译器内建类型。直接 `String` 值在 WASM 中使用 packed `i64` 句柄：低 32 位为 UTF-8 指针，高 32 位为字节长度；字符串字面量降低为确定性的只读 UTF-8 字节区域。
- `Slice<T>` 的 WASM 表示为 `{ ptr: i32, len: i32 }`，其中 `len` 是元素数量。v1 不要求数组字面量、索引、扩容或集合运行时。
- host module 名为 `maodie`，最小保留 imports 为 `panic(ptr: i32, len: i32)`、`debug_string(ptr: i32, len: i32)`、`debug_i32(value: i32)`、`debug_bool(value: i32)` 和 `debug_log_end()`。
...
- WASM v1 布局先把 `i32`、`bool`、切片句柄和 enum/Result 值映射为 `i32`；直接 `String` 值使用 packed `i64` 指针/长度句柄；一字段 variant 使用低 8 位 tag 和高位 payload 编码。
- 生成 WAT debug dump、WASM binary、线性内存 export，以及 `maodie.panic` / debug chunk imports。
